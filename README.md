...
| **🧠 Virtual Memory Paging** | Active task lists grow indefinitely, bloating the context window and wasting thousands of tokens per run. | **Resilient Archiving** — Automatically pages completed tasks into `task_archive.md` at 30,000 words, fetching specific line ranges dynamically on-demand to keep context clean. |
...
*   **📋 Persistent Workspace & Session Memory (Virtual Memory Paging):** Automatically maintains a standard task checklist (`task.md`) and project context guidelines (`AGENTS.md`). Implements a unique "Virtual Memory Paging" mechanism: when the file reaches 30,000 words, older completed tasks are archived to `task_archive.md`. The agent only pages/loads specific line ranges of the archive dynamically on-demand, keeping the active context window clean and saving tokens.
