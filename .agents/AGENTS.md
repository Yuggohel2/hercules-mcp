...
- **Rule**: **Task List Size Limit & Archiving:** To prevent context window bloat, the active `task.md` (both global and project-specific) must be kept compact (typically under 150 lines or 2,000 words). If it exceeds this size, the agent MUST automatically archive older completed tasks/phases into a sibling archive file named `task_archive.md` (e.g., `WORKSPACE_ROOT/task_archive.md` or `PROJECT_DIR/task_archive.md`). The active `task.md` should retain only a brief, high-level summary of archived phases (e.g., "*Phases 1-3 completed and archived in task_archive.md*") and the detailed list of active or recently completed tasks.
